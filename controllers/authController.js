...
exports.forgotPassword = async (req, res) => {
  try {
    ...
    if (!alumni) {
      return res.json({ message: "If an account exists, an OTP will be sent shortly." });
    }

    const previousRequest = otpRequests.get(email.toLowerCase());
    ...
    otpStore.set(email.toLowerCase(), { otpHash: crypto.createHash("sha256").update(otp).digest("hex"), expiresAt, attempts: 0 });
    otpRequests.set(email.toLowerCase(), Date.now());
...
exports.resetPassword = async (req, res) => {
  try {
    ...
    const submittedHash = crypto.createHash("sha256").update(otp.toString()).digest("hex");
    if (stored.attempts >= 5 || submittedHash !== stored.otpHash) {
      if (stored.attempts < 5) stored.attempts += 1;
